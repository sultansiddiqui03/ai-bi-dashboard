...
import { Eraser, Copy, ArrowUpDown, Type, Hash, Trash2, CheckCircle2, AlertTriangle, Wrench } from 'lucide-react';
...
export default function DataCleaningPanel({ data, columns, stats, onDataUpdate }) {
  ...
  const fillMissingValues = (col, method) => {
    ...
    if (method === 'mean' && stat?.type === 'numeric') {
      fillValue = stat.mean;
    } else if (method === 'median' && stat?.type === 'numeric') {
      fillValue = stat.median;
    } else if (method === 'mode') {
      if (stat?.type === 'categorical' && stat.topValues?.length > 0) {
        fillValue = stat.topValues[0].value;
      } else {
        fillValue = 0;
      }
    } else if (method === 'zero') {
      fillValue = stat?.type === 'numeric' ? 0 : '';
    } else {
      return;
    }
                    ...
                    <div key={col} className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.02] border border-[var(--border-subtle)]">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-[var(--text-muted)]">
                          {missingCount} missing ({((missingCount / data.length) * 100).toFixed(1)}%)
                      ...
                      <div className="flex gap-1 shrink-0">
                        {isNumeric && (
                          <>
                            <button
                              onClick={() => fillMissingValues(col, 'mean')}
                              className="px-2 py-1 rounded text-[10px] font-medium bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 transition-all"
                              title="Fill with mean"
                            >
                              Mean
                            </button>
                            <button
                              onClick={() => fillMissingValues(col, 'median')}
                              className="px-2 py-1 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all"
                        ...
                        <button
                          onClick={() => fillMissingValues(col, 'mode')}
                          className="px-2 py-1 rounded text-[10px] font-medium bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 transition-all"
                          title="Fill with most common value"
                        >
                          Mode
                        </button>
                        <button
                          onClick={() => removeRowsWithMissing(col)}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-40 overflow-y-auto">
              {columns.slice(0, 12).map(col => {
                ...
                return (
                  <button
                    key={col}
                    onClick={() => convertColumnType(col, isNumeric ? 'string' : 'number')}
